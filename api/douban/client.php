...
{ 
    /** 
     * 构造函数 
     * @param mixed $akey 豆瓣给你的 APP KEY (Consumer_Key ) 
     * @param mixed $skey 豆瓣给你的 APP SECRET (Consumer_Secret_Key ) 
...
//////////////////////////////////////用户相关////////////////////////////////////////////
     ...
     * 通过ID得到用户的好友列表 
     */ 
    function friends( $page = 1 , $id ) 
    {
		$p = array(
			'id' => $id,
			'page' => $page
		);
        return $this->oauth-> get( 'http://api.fanfou.com/users/friends.json' ,  $p ); 
    } 

    /** 
     * 通过ID得到用户的粉丝列表 
     */ 
    function followers( $page , $id ) 
    { 
		$p = array(
			'id' => $id,
			'page' => $page
		);
        return $this->request_with_uid( 'http://api.fanfou.com/users/followers.json' ,  $p ); 
    } 
    ...
    { 
        return $this->request_with_pager( 'http://api.fanfou.com/direct_messages.json' , $page , $count ); 
    } 
    ...
    { 
        return $this->oauth->post( 'http://api.fanfou.com/direct_messages/destroy/' . $did . '.json' ); 
    } 
    ...
    function verify_credentials() 
    { 
        $douban_info = $this->oauth->get( 'http://api.douban.com/people/@me' ,array("alt"=>"json"));
        $douban_info = json_decode($douban_info);var_dump($douban_info);
        return array(
            "id" => $douban_info->title,
            "screen_name" => $douban_info->title,
            );
    }

    function miniblog_saying($content){
        $param = array(); 
