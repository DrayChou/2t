...
$o = new OAuth( $config["api"]["douban"]["consumer_key"] , $config["api"]["douban"]["consumer_secret"] , $_SESSION['temp']['oauth_token'] , $_SESSION['temp']['oauth_token_secret']  );
...
echo '<br>以后你就可以拿着上面的两个家伙访问豆瓣了。下面是你的消息列表，直接都是json数据,小心眼花哦<br>';

//创造一个新的请求

$ff_user = new Client( $config["api"]["douban"]["consumer_key"] , $config["api"]["douban"]["consumer_secret"] , $last_key['oauth_token'] , $last_key['oauth_token_secret']  );
$userinfo = $ff_user -> verify_credentials();
echo $ff_user -> miniblog_saying("测试豆瓣广播");
...
set_config("douban", $userinfo["screen_name"], $last_key, $config["data_directory"]);
